...
#if IS_ENABLED(CONFIG_ZMK_CUSTOM_SETTINGS)
ZMK_CUSTOM_SETTING_DEFINE(kscan_diagnostics_sample_bool, "cormoran__kscan_diagnostics",
...
static int handle_sample_request(const cormoran_kscan_diagnostics_SampleRequest *req,
...
static bool kscan_diagnostics_rpc_handle_request(const zmk_custom_CallRequest *raw_request,
    ...
    if (!pb_decode(&req_stream, cormoran_kscan_diagnostics_Request_fields, &req)) {
        ...
        cormoran_kscan_diagnostics_ErrorResponse err =
            cormoran_kscan_diagnostics_ErrorResponse_init_zero;
    ...
    case cormoran_kscan_diagnostics_Request_sample_tag:
...
static int handle_sample_request(const cormoran_kscan_diagnostics_SampleRequest *req,
    ...
    cormoran_kscan_diagnostics_SampleResponse result =
        cormoran_kscan_diagnostics_SampleResponse_init_zero;

    snprintf(result.value, sizeof(result.value), "Hello from firmware! Received: %d", req->value);

    resp->which_response_type = cormoran_kscan_diagnostics_Response_sample_tag;
