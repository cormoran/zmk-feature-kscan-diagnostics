...
| P0-5 | `skills/zmk-module-design/SKILL.md`: a design-phase reference condensing the RPC/custom-settings/web API surfaces (extracted from `zmk/studio/custom.h`, `custom_settings.h`, and the react hook), the constraints that shape protocol design (shared static response buffer, TX/RX budgets, chunking/notification patterns, native_sim requirement), a DESIGN.md checklist, and the proven phase plan. Purpose: even a strong model designing a module burns thousands of tokens re-reading dependency headers per repo; this makes the design phase a single ~150-line read. | 4, 6 |
