...
import type { CellWiring, DiagnosisKeyEvent, PositionCoverage } from "../../src/diagnosis/types";
...
function colLine(col: number): GpioPin {
  return { index: 100 + col, port: "gpio1", pin: col, activeLow: true, dtFlags: 0 };
}

export function makeCells(deviceType: KscanDriverType = KscanDriverType.MATRIX): CellWiring[] {
...
export function makeCoverage(overrides: Record<number, Partial<PositionCoverage>> = {}): PositionCoverage[] {
...
export function makeStatsEntry(overrides: Partial<PositionStatsEntry> = {}): PositionStatsEntry {
  return {
    position: 0,
    presses: 1,
    releases: 1,
...
export function makeStatsMap(
  ...
  for (const override of entries) {
    ...
    map.set(override.position, makeStatsEntry({ ...map.get(override.position), ...override }));
  }
  return map;
}

export function makeEvent(overrides: Partial<DiagnosisKeyEvent> = {}): DiagnosisKeyEvent {
